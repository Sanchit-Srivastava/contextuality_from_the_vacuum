...
def test_weyl_product_rule_all_contexts():
    """Assert the Weyl product rule holds exhaustively within each context."""
    for c in range(len(A)):
        assert check_weyl_product_rule_in_context(c, exhaustive=True)


def test_weyl_commutation_all_contexts():
    """Assert Weyl(u) and Weyl(v) commute for all u,v in each context."""
    for c in range(len(A)):
        assert check_algebra(c, exhaustive=True)
